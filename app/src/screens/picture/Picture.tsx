...
import { GET_TODAY_PICTURE } from './queries';
...
function PictureScreen({ route, navigation }: Props) {
  ...
  const _ = useQuery(GET_TODAY_PICTURE, {
    onCompleted: (data) => {
      // eslint-disable-next-line prefer-destructuring
      const todayPicture: PictureType = data.todayPicture;
      todayPicture.explanation = data.todayPicture.description;
      setResponse(todayPicture);
    },
    onError: () => {
      setError(true);
    },
  ...
  useEffect(() => {
    if (response && JSON.stringify(response) !== JSON.stringify({})) {
      getSimilars();
      setLoading(false);
    }
  }, [response]);
